<?php
class ControllerExtensionModulecodezeeltabs extends Controller {
	public function index($setting) {
		$this->load->language('extension/module/codezeeltabs');
		  ...
		  $i = 0;
			foreach ($results as $result) {
				...
				if($categories){
					...
					$categories_id = $categories[0]['category_id'];
				}else{
				    $category_title = '';
				}
				...
				$data['specialproducts'][] = array(
					...
					'category_href' => $this->url->link('product/category', 'path=' . $categories_id),
		...
		if ($results) {
		  $i = 0;
			...
			return $this->load->view('extension/module/codezeeltabs', $data);
	}
}